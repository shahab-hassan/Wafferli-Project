...
import { GetFeaturedServiceDetails } from "@/features/slicer/AdSlice";
...
export default function ServiceDetailPage({
  ...
  const fetchProductDetails = async () => {
    ...
    try {
      const res = await dispatch(
        GetFeaturedServiceDetails(adId) as any
      ).unwrap();
  ...
  console.log("serviceDetails", serviceData);

  const responseData = serviceData as any;

  const productData = responseData?.service || {};
  const sellerData = responseData?.seller || {};
  const relatedProducts = responseData?.relatedServices || [];
              ...
              <div className="relative h-80 md:h-96">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  <Badge className="bg-purple-100 text-purple-700 backdrop-blur-sm">
                    {productData.serviceType || "Product"}
                  </Badge>
                </div>
