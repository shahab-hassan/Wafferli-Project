...
export default function MarketplaceFilterSidebar({
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    if (isDragging === "min") {
      setTempRange([Math.min(value, tempRange[1] - 50), tempRange[1]])
    } else {
      setTempRange([tempRange[0], Math.max(value, tempRange[0] + 50)])
    }
  }
            ...
            <div className="space-y-6">
              ...
              <div>
                <button
                  ...
                  className="flex items-center justify-between w-full text-left"
                >
                  <h4 className="text-sm font-medium text-foreground">{t("marketplace.filters.brands")}</h4>
                  <ChevronDown
                    className={cn("w-4 h-4 transition-transform", expandedSections.brands && "rotate-180")}
                  />
                </button>
                {expandedSections.brands && (
                  <div className="mt-3 space-y-3">
                    {[
                      ...
                      <label key={brand.key} className="flex items-center space-x-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={activeFilters.brands.includes(brand.key)}
                          onChange={() => toggleFilter("brands", brand.key)}
              ...
              <div>
                <button
                  ...
                  className="flex items-center justify-between w-full text-left"
                >
                  <h4 className="text-sm font-medium text-foreground">{t("marketplace.filters.condition")}</h4>
                  <ChevronDown
                    className={cn("w-4 h-4 transition-transform", expandedSections.condition && "rotate-180")}
                  />
                </button>
                {expandedSections.condition && (
                  <div className="mt-3 space-y-3">
                    {[
                      { key: "brand_new", label: t("marketplace.filters.brandNew") },
                      { key: "used_excellent", label: t("marketplace.filters.usedExcellent") },
                      { key: "used_good", label: t("marketplace.filters.usedGood") },
                      { key: "refurbished", label: t("marketplace.filters.refurbished") },
                    ].map((condition) => (
                      <label key={condition.key} className="flex items-center space-x-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={activeFilters.condition.includes(condition.key)}
                          onChange={() => toggleFilter("condition", condition.key)}
                          className="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary"
                        />
