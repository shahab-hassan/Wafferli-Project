...
export default function FilterSidebar({
  ...
  const categories = useMemo(() => [
    { name: t("explore.categories.restaurants"), color: "bg-pink-100 text-pink-600 border-pink-200", icon: "⭐" },
    { name: t("explore.categories.shopping"), color: "bg-pink-100 text-pink-600 border-pink-200", icon: "🛍️" },
    { name: t("explore.categories.cafes"), color: "bg-orange-100 text-orange-600 border-orange-200", icon: "☕" },
    { name: t("explore.categories.landmarks"), color: "bg-gray-100 text-gray-600 border-gray-200", icon: "🏛️" },
    { name: t("explore.categories.parks"), color: "bg-green-100 text-green-600 border-green-200", icon: "🌳" },
    { name: t("explore.categories.family"), color: "bg-blue-100 text-blue-600 border-blue-200", icon: "👨‍👩‍👧‍👦" },
    { name: t("explore.categories.cultural"), color: "bg-purple-100 text-purple-600 border-purple-200", icon: "🎭" },
    { name: t("explore.categories.hotels"), color: "bg-cyan-100 text-cyan-600 border-cyan-200", icon: "🏨" },
    { name: t("explore.categories.entertainment"), color: "bg-purple-100 text-purple-600 border-purple-200", icon: "🎪" },
    { name: t("explore.categories.weddings"), color: "bg-pink-100 text-pink-600 border-pink-200", icon: "💍" },
  ], [t])
  ...
  const features = useMemo(() => [
    ...
    { key: "wheelchair_accessible", label: t("explore.wheelchair_accessible") },
        ...
        <Card className="bg-white border">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold text-foreground">{t("explore.filters")}</h3>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="space-y-6">
              {/* Location Filter */}
              <div>
                <h4 className="text-sm font-medium text-foreground mb-3">{t("explore.location")}</h4>
                <Dropdown
              ...
              <div>
                ...
                >
                  <h4 className="text-sm font-medium text-foreground">{t("explore.explore_categories")}</h4>
                  <ChevronDown
                    className={cn("w-4 h-4 transition-transform", expandedSections.categories && "rotate-180")}
                  />
                </button>
              ...
              <div>
                <button
                  ...
                  className="flex items-center justify-between w-full text-left"
                >
                  <h4 className="text-sm font-medium text-foreground">{t("explore.features_amenities")}</h4>
                  <ChevronDown
                  ...
                  <div className="mt-3 space-y-3">
                    {features.map((feature) => (
                      <label key={feature.key} className="flex items-center space-x-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={activeFilters.categories.includes(feature.key)}
                          onChange={() => toggleFilter("categories", feature.key)}
              ...
              <div>
                <button
                  ...
                  className="flex items-center justify-between w-full text-left"
                >
                  <h4 className="text-sm font-medium text-foreground">{t("explore.listing_type")}</h4>
                  <ChevronDown
