...
React.useEffect(() => {
  ...
  if (mobileLayout) {
    return (
      <div role="dialog" aria-label={t("profile")}
        dir={dir}
        ...
        <div className="p-4">
          <div className={cn("flex items-center gap-3", flexDir)}>
            ...
            <div className="min-w-0">
              <div className={cn("font-medium text-foreground truncate", alignClass)}>{user.name}</div>
              <div className={cn("text-xs text-gray-600 truncate", alignClass)}>{user.email}</div>
              <Link
                href={`/${currentLocale}/profile`}
                onClick={onClose}
                className={cn(
                  "text-xs mt-1 inline-block hover:underline",
                  currentLocale === "ar" ? "ml-auto" : ""
                )}
              >
                {t("viewPublicProfile")}
              </Link>
        ...
        <div className="p-4">
          <div className="rounded-lg overflow-hidden mb-3" aria-hidden>
            <div className="flex items-center gap-3 px-3 py-3 bg-gradient-to-r from-primary to-secondary text-white rounded-lg">
              ...
              <div className="min-w-0">
                <div className="font-semibold text-lg leading-tight">{user.points?.toLocaleString()}</div>
          ...
          >
            {role === "user" ? t("switchToSeller") : t("switchToUser")}
          </button>
        </div>
...
return (
  <div
    ...
    className={cn(
      ...
      ((): string => {
        // default anchor
        const defaultAnchor = currentLocale === "ar"
          ? "left-0 origin-top-left"
          : "right-0 origin-top-right";

        // flipped anchor (mirror)
        const flippedAnchor = currentLocale === "ar"
          ? "right-0 origin-top-right"
          : "left-0 origin-top-left";
