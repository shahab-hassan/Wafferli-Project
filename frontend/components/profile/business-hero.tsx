...
const IMAGES = [
  "/placeholder.svg?height=360&width=800",
  "/placeholder.svg?height=360&width=800",
  "/placeholder.svg?height=360&width=800",
]
  ...
  return (
    <section className="relative w-full h-[220px] md:h-[300px] lg:h-[360px] rounded-xl overflow-hidden border border-white/30 mt-4">
      ...
      <div className="absolute top-4 right-4 flex items-center gap-3 z-20">
        ...
        >
          <Heart className={`w-5 h-5 ${liked ? "text-primary" : "text-gray-600"}`} />
        </button>
        ...
        {IMAGES.map((_, i) => (
          <button
            ...
            className={`transition-all w-2 h-2 rounded-full ${
              i === index ? "w-8 h-2 rounded-full bg-primary" : "bg-white/70 border border-white/40"
            }`}
